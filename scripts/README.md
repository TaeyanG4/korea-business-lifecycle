...
- `prepare_kaggle_row_release.py`: verifies the canonical PERMIT parent and prepares the approved 3,010,802-row × 26-column Kaggle release as both CSV and Parquet with source summary, schema, dictionary, docs, manifest, and pinned dataset metadata.
- `verify_kaggle_row_release.py`: independently re-parses the prepared CSV, verifies exact row/column parity with Parquet, and checks the release payload hashes and package contract.
