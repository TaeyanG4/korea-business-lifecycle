...
- `prepare_kaggle_release.py`: verifies the approved aggregate hash and prepares an aggregate-only Kaggle upload directory under ignored `data/local/kaggle_release/`; `--owner` writes upload-ready `dataset-metadata.json`.
