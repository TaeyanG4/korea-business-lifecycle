...
- `kaggle_dataset_maintenance_v2.json`: current Kaggle-maintenance evidence for the consolidated 3,010,802-row snapshot: Version 2, unchanged 3,010,802-row payload, structured Overview, cover, sources, monthly cadence, and the completed public Quickstart notebook. File/column descriptions are authored and the 8-file/56-column Data Explorer sync dry-run is verified. A live write attempt reached Kaggle but returned HTTP 401 because the internal Data Explorer save route requires an authenticated web session; persistence therefore remains pending at Usability 0.8235294 (`EDIT_FILE_INFO`, `EDIT_COLUMN_DESCRIPTION`).
