...
- `kaggle_dataset_maintenance_v2.json`: current Kaggle-maintenance evidence for the consolidated 3,010,802-row snapshot: Version 2, unchanged 3,010,802-row payload, structured Overview, cover, sources, monthly cadence, Quickstart v6, and the public Regional Market Map v1. It also freezes the monthly review checklist and required Version Notes fields without allowing row duplication or artificial payload shrink. File/column descriptions are authored and the 8-file/56-column Data Explorer sync dry-run is verified. The internal Data Explorer write route returns HTTP 401 without an authenticated web session; the official dataset-metadata update request was also accepted but left Data Explorer descriptions at 0/8 files and 0/56 columns. Persistence therefore remains pending at Usability 0.8235294 (`EDIT_FILE_INFO`, `EDIT_COLUMN_DESCRIPTION`).
