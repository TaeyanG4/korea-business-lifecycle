...
current snapshot 취득/프로파일링과 bounded history audit이 완료되었고, `PERMIT` parent build `permit-v1-9908225df465e2ff`와 별도 WGS84 sidecar `permit-geo-v1-c4af8799de0283bb`는 모두 **3,010,802행** 기준 production materialization·독립 검증까지 완료했습니다. lifecycle semantic hard rules도 유지합니다: `03`은 irreversible terminal이 아니고 `05`는 unmapped입니다. 삭제 32개 × 3 source × 4 dates의 **384-request count-only probe**도 384/384 완료했습니다. 공식 2026-07-01 변경표의 신규 32·삭제 32를 사용해 date-effective current-state authority domain은 **개편 전 244 = current 244 - new 32 + deleted 32**, **개편 후 current 244 only**로 고정했습니다. API queryability 자체는 이 membership을 결정하지 않습니다. production history cadence는 **MONTHLY_ANCHOR_PLUS_END** 10개 날짜, 최대 gap 31일, current-scale planning **301,258–308,380 requests**, 실행 hard cap 400,000으로 승인했습니다. 전국 수집은 아직 실행하지 않았으며 resumable runner와 complete-7,320-snapshot gate, bucketed episode materializer/independent verifier까지 준비했습니다. redistribution 서면확인 질문/문의 경로도 준비했지만 외부 공개 승인은 아직 아닙니다.
